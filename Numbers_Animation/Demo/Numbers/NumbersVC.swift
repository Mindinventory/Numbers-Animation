...
final class NumbersVC: UIViewController {
    ...
    private let smallNumbersData = [SmallNumbers(image: UIImage(named: "ic_small0") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_BigOne") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small2") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small3") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small4") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small5") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small6") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small7") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small8") ?? UIImage()), SmallNumbers(image: UIImage(named: "ic_small9") ?? UIImage())]
...
extension NumbersVC {
    ...
    private func configure() {
        ...
        //constCollNumberHeight.constant = CScreenWidth * (180/414)
        
        setAttributedTextonLbls()
        
        CMainThread.asyncAfter(deadline: .now() + 0.2, execute: {
            
            self.animateViews(btnBack: self.btnBack, lblNumbers: self.lblNumbers, lblNumberDesc: self.lblNumberDesc, vcView: self.view, constBtnBackTop: self.constBtnBackTop, collNumbers: self.collNumbers, smallNumbersData: self.smallNumbersData)
        })
    }
    ...
    private func setAttributedTextonLbls() {
        ...
        attrStr.addAttribute(.foregroundColor, value: UIColor.init(red: 255/255, green: 111/255, blue: 77/255, alpha: 1), range: NSRange(location: 1, length: 1))
        attrStr.addAttribute(.foregroundColor, value: UIColor.init(red: 255/255, green: 179/255, blue: 87/255, alpha: 1), range: NSRange(location: 5, length: 3))
        attrStr.addAttribute(.font, value: UIFont.systemFont(ofSize: CScreenWidth * (30/414), weight: .black), range: NSRange(location: 1, length: 1))
