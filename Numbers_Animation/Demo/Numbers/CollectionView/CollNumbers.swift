...
extension CollNumbers: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if let numbersCell = collectionView.dequeueReusableCell(withReuseIdentifier: NumbersCell.identifier,
...
extension CollNumbers {
    ...
    private func animateCell(row: Int, numberCell: NumbersCell) {
        ...
        if let numbersVC = UIApplication.topViewController() as? NumbersVC {
            ...
            numbersVC.vwNumberAnimate.frame =
                self.superview?.convert(numberCell.frame, from: self) ?? CGRect()
            imgView.frame = CGRect(x: 0, y: 0, width: numbersVC.vwNumberAnimate.frame.width,
                                   height: numbersVC.vwNumberAnimate.frame.height)
